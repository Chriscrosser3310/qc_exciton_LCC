...
def log_block_sizes_for(loader_name: str, selection_bits: int):
    ...
    if loader_name == "QROM":
        return [None]
    return [log_size for log_size in LOG_BLOCK_SIZES if log_size <= selection_bits]
...
def plot_metric(metric_name: str, ylabel: str, ax=None):
    if ax is None:
...
def comparison_figure():
    ...
    plot_metric("toffoli_count", "Toffoli count", ax=axes[0])
    plot_metric("ancilla_count", "Ancilla count", ax=axes[1])
    fig.suptitle("Data-loading scheme comparison", y=1.03)
    fig.tight_layout()
    return fig


def save_comparison_image(output_path="data_loading_comparison.png"):
