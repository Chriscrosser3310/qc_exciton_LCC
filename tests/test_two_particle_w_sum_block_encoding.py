...
from integrations.qualtran.block_encoding.two_particle_w_sum_block_encoding import (
...
def test_two_particle_w_sum_block_encoding_signature():
    # num_pairs=2 -> 4 system registers, C(4,2)=6 selection terms.
    w = 0.2 * np.ones((2, 2, 1, 1), dtype=np.float64)
    bloq = build_two_particle_w_sum_block_encoding(
        num_pairs=2,
        D=1,
        L=2,
        R_c=0,
    ...
    assert isinstance(bloq, TwoParticleWSumBlockEncoding)
    reg_names = [r.name for r in bloq.signature]
    assert reg_names == ["q", "sel", "m", "l", "r0", "r1", "r2", "r3"]


def test_two_particle_w_sum_block_encoding_m1_sign():
    # m=1 -> only two registers and one pair term (0,1), which is cross-partition and thus negative.
    # Therefore postselected block equals -W on (r0, r1).
    w = np.array(
        [
            [[[0.2]], [[0.3]]],
            [[[0.4]], [[0.7]]],
        ...
        dtype=np.float64,
    )  # shape (L,L,1,1), D=1, L=2, R_c=R_loc=0
    bloq = build_two_particle_w_sum_block_encoding(
        num_pairs=1,
        D=1,
        L=2,
    ...
    expected_diag = np.array([-w[0, 0, 0, 0], -w[0, 1, 0, 0], -w[1, 0, 0, 0], -w[1, 1, 0, 0]])
    expected = np.diag(expected_diag.astype(np.complex128))
    assert np.allclose(block, expected, atol=1e-4)


def test_two_particle_w_sum_block_encoding_controlled_specialization():
    w = 0.2 * np.ones((2, 2, 1, 1), dtype=np.float64)
    bloq = build_two_particle_w_sum_block_encoding(
        num_pairs=1, D=1, L=2, R_c=0, R_loc=0, W_table=w, entry_bitsize=8
    )
    ctrl = bloq.controlled()
    assert isinstance(ctrl, TwoParticleControlledWSumBlockEncoding)

    direct_ctrl = build_two_particle_controlled_w_sum_block_encoding(
        num_pairs=1, D=1, L=2, R_c=0, R_loc=0, W_table=w, entry_bitsize=8
    )
    assert isinstance(direct_ctrl, TwoParticleControlledWSumBlockEncoding)
